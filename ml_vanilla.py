...
def update_u_cap(R,mean,delta,N,u_cap,Arms):
    u_now=pull(Arms,mean)
    return (u_now+u_cap*(R-1))/(R),1

def vanila_algo(delta,cluster,required,Arms,merging,pulls,v):

    u_cap=[[pull(Arms,i),i] for i in Arms]
    count=len(Arms)
    R=0
    ...
    while(check_true(required,cluster,u_cap,count,pulls,v)):
        ...
        for i in range(len(Arms)):
            if u_cap[i][1]!=-1:
                u_cap[i][0],c=update_u_cap(R,u_cap[i][1],delta,len(Arms),u_cap[i][0],Arms)
                count+=c
